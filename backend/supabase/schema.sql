-- ============================================================
-- SmartCup League — Full Database Schema
-- Single file — run this in: Supabase Dashboard → SQL Editor → New Query
-- ============================================================
...
-- ============================================================
-- TABLE: vara_prices
-- Historical VARA/USD price snapshots fetched from CoinGecko.
-- The backend caches in-memory (5 min TTL) and falls back to
-- the latest row here if CoinGecko is unreachable.
-- ============================================================
...
-- Immutable log — one row per wallet + match bet.
-- UNIQUE(wallet_address, match_id) prevents double-counting.
-- ============================================================
...
-- ============================================================
-- TABLE: claim_events
-- Immutable log — one row per wallet + match claim.
-- amount_planck is the balance delta observed on-chain (approx).
-- is_exact: true when the user predicted the exact final score.
-- ============================================================
...
-- ============================================================
-- ROW LEVEL SECURITY
-- Public read (leaderboard is public). Only the backend
-- service role can write.
-- ============================================================
alter table public.vara_prices        enable row level security;
alter table public.prediction_events  enable row level security;
alter table public.claim_events       enable row level security;

-- vara_prices
create policy "public_read_vara_prices"
    on public.vara_prices for select using (true);

create policy "service_role_insert_vara_prices"
    on public.vara_prices for insert to service_role with check (true);

-- prediction_events
create policy "public_read_prediction_events"
    on public.prediction_events for select using (true);

create policy "service_role_insert_prediction_events"
    on public.prediction_events for insert to service_role with check (true);

-- claim_events
create policy "public_read_claim_events"
    on public.claim_events for select using (true);

create policy "service_role_insert_claim_events"
    on public.claim_events for insert to service_role with check (true);

-- ============================================================
-- VIEW: latest_vara_price
-- Always returns the single most recent VARA/USD price.
-- ============================================================
...
-- ============================================================
-- VIEW: user_leaderboard_stats
-- Aggregated per-wallet stats, computed from event tables.
-- No running totals to maintain — always consistent.
-- ============================================================
...
full outer join (
    select
        ...
        sum(amount_planck)                  as total_claimed_planck,
...
-- ============================================================
-- VIEW: match_pool_stats
-- Per-match betting distribution (home / draw / away).
-- Used to display pool breakdown bars in the UI.
-- ============================================================
...
-- ============================================================
-- FUNCTION: cleanup_old_prices()
-- Keeps only 30 days of price history to control table size.
-- ============================================================
...
-- ============================================================
-- OPTIONAL: Schedule cleanup with pg_cron (runs daily at 03:00 UTC)
-- Uncomment if pg_cron is enabled on your Supabase project.
-- ============================================================
-- select cron.schedule(
--     'cleanup-old-vara-prices',
--     '0 3 * * *',
--     $$ select public.cleanup_old_prices(); $$
-- );
